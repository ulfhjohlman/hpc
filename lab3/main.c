...
void compute(double * coord, int * count,int * low_count, int * real_count,int k,int t){
    double x,y,z;
    double dist;
    int i,j;
    #pragma omp parallel num_threads(t)
    {
        ...
        int * real_count_private= calloc(3465,sizeof(int));
		...
		//#pragma omp atomic
                //count[(int) dist]++;
                real_count_private[(int)(0.5+sqrt(dist)*100)]++;
        if(dist<1){
			low_count_private[ (int)(0.5+sqrt(dist)*100)]++;
		}
        else{
	...
	#pragma omp critical
	{
        ...
        for(int l=0;l<3465;l++){
			real_count[l]+=real_count_private[l];
		}
	}
    }
    return;
...
int main(int argc, char *argv[]){
    ...
    real_count = calloc(3465,sizeof(int));

    compute(coord,count,low_count,real_count,k,t);

    int i,j;
    for(i=0;i<100;i++){
        if(low_count[i]>0){
            printf("%.2f %d   real: %d\n", i*0.01 , low_count[i],real_count[i]);
        }
    }
    int c=0;
    j=100;
    ...
    for(i=100;i<MAX_COUNT;i++){
        sqrtBin = sqrt(i/100.0)*100.0;
        if(sqrtBin > j + 0.5){
            printf("%.2f %d    real: %d\n",j*0.01 , c, real_count[j]);
            c=0;
            j++;
        }
        c+=count[i];
